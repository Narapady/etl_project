...
def stage_fastfood(
    s3: S3AWS,
    dirname: str,
    src_bucket: str = "s3-bucket-clean-usda",
    des_bucket: str = "s3-bucket-staged",
):
...
def stage_foodnutrient_estimates(
    s3: S3AWS,
    dirname: str,
    src_bucket: str = "s3-bucket-clean-usda",
    des_bucket: str = "s3-bucket-staged",
...
def stage_foodexpenditure(
    ...
    monthly_df["year"] = monthly_df["year"].map(
        lambda x: pd.to_datetime(f"{x}-01-01"))
    expense_df["year"] = expense_df["year"].map(
        lambda x: pd.to_datetime(f"{x}-01-01"))
...
def stage_priceindex(
    ...
    for file in filenames:
        ...
        if index_type == "consumer":
            df = df.rename(
                columns={"Consumer Price Index item": "price-index-item"})
        elif index_type == "producer":
            df = df.rename(
                columns={"Producer Price Index item": "price-index-item"})
        result_df = pd.concat([result_df, df])
...
def stage_foodavailability(
    ...
    for file in paths:
        ...
        if file == "totals.csv" or file == "percents.csv":
            ...
            if file in [
                "dairy/romano-cheese.csv",
                "dairy/ricotta-cheese.csv",
                "dairy/parmesan-cheese.csv",
                "dairy/provolone-cheese.csv",
            ]:
    ...
    result_df["year"] = result_df["year"].map(
        lambda x: pd.to_datetime(f"{x}-01-01"))
...
def stage_obesity_kaggle(
    s3: S3AWS,
    dirname: str,
    src_bucket: str = "s3-bucket-raw-kaggle",
    des_bucket: str = "s3-bucket-staged",
):
    ...
    for file in filenames:
        if file == "Obesity_GDP_PanelData.csv" or file == "obesity-cleaned.csv":
            ...
            if "Unnamed: 0" in list(df.columns):
                df = df.drop("Unnamed: 0", axis=1)
                df["Obesity (%)"] = df["Obesity (%)"].apply(
                    lambda x: x.split(" ")[0])
                df["Obesity (%)"] = df["Obesity (%)"].replace("No", np.nan)
            else:
                df["State_code"] = df["State"].apply(
                    lambda x: get_state_code(x))
                df = df.drop(
                    columns=[
                        "Adult.Obesity",
                        "Poverty.Rate",
